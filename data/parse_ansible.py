...
def main():
    module_keys = ('module', 'short_description',
                   'options', 'deprecated', 'suboptions')
    ...
    fn = os.path.join(__path__, 'ansibleData.js')
    ...
    with open(fn, 'r') as f:
        content = f.read()
    with open(fn, 'w') as f:
        f.write('var ansibleData = ' +
                p.sub(r'<code>{\1}</code>', content))
